    ...
    def check_changelog(version):
        with open(_CHANGELOG_FILE_PATH) as fd:
            ...
            if last_version != version:
                raise ValueError('unexpected version in {}: {} (found) != {} (expected)'.format(_CHANGELOG_FILE_PATH, last_version, version))
            if last_date > _NOW:
                raise ValueError('invalid date in {}: {} (date is in the future)'.format(_CHANGELOG_FILE_PATH, last_date.strftime('%Y-%m-%d')))
        ...
        with open(_INIT_FILE_PATH) as fd:
            for line in fd:
                ...
                if found is not None:
                    ...
                    if last_version != version:
                        raise ValueError('unexpected version in {}: {} (found) != {} (expected)'.format(_INIT_FILE_PATH, last_version, version))
...
def parse_args(argv):
    ...
    parser.add_argument('-c', '--check-only', action='store_true', help='instead of updating tag info, just check if it matches given tag or version')
