...
def test_create_call_object_with_name_only():
    call = Call('foo')
    assert str(call) == 'foo()'
...
def test_create_call_object_with_name_and_positional_args():
...
def test_two_call_objects_are_equal_if_names_and_params_are_equal(first, second):
    assert first == second
...
def test_two_call_objects_are_not_equal_if_they_have_different_names_different_arg_values_or_different_args_count(first, second):
    assert first != second
...
def test_call_can_be_created_with_name_composed_of_identifiers_glued_with_a_period(namespaced_name):
