...
class TestLocationInfo:
    ...
    def test_issue_deprecation_warning_if_location_info_is_used(self):
        ...
        (first,) = rec
        assert (
            str(first.message) == "'mockify.core.LocationInfo' is deprecated since version 0.13 "
...
class TestCall:
    ...
    def test_name_must_be_a_valid_python_identifier(self, invalid_name):
        with pytest.raises(TypeError) as excinfo:
            Call(invalid_name)
        assert str(excinfo.value) == "Mock name must be a valid Python identifier, got {!r} instead".format(
            invalid_name
        )

    @pytest.mark.parametrize("namespaced_name", ["foo.bar", "foo.bar.baz", "foo.__call__", "foo.bar.baz.__spam"])
